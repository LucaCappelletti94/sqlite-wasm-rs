...
fn main() {
    ...
    let (default_dir, source_name, header_name) = (
        sqlcipher_amalgamation::source_dir().to_path_buf(),
        sqlcipher_amalgamation::WASM_SOURCE_FILE,
        sqlcipher_amalgamation::HEADER_FILE,
    );
    #[cfg(feature = "sqlite3mc")]
    let (default_dir, source_name, header_name) = (
    ...
    #[cfg(feature = "bindgen")]
    {
        ...
        if update_bindgen {
            ...
            // sqlcipher-amalgamation regenerates the sqlcipher bindings next to its sources.
    ...
    #[cfg(all(not(feature = "bindgen"), feature = "sqlcipher"))]
    {
        let output = PathBuf::from(std::env::var("OUT_DIR").expect("OUT_DIR env not set"))
            .join("bindgen.rs");
        std::fs::copy(
            sqlcipher_amalgamation::source_dir().join(sqlcipher_amalgamation::WASM_BINDINGS_FILE),
            output,
        )
...
fn compile(source: &Path) {
    ...
    // Shared memory needs bulk memory too, and clang 18 strips atomics from objects built without it.
